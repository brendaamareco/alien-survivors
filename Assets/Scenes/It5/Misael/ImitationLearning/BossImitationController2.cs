...
public class BossImitationController2 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (EtIsCovered(m_boss.transform.localPosition, enemy.transform.localPosition, m_player.transform.localPosition))
            {
                estaDetrasDeAlgunEnemigo = true;
                float enemyAngle = GetEtAngle(m_boss.transform.localPosition, enemy.transform.localPosition, m_player.transform.localPosition);
                if (enemyAngle > angle) 
                {
                    angle = enemyAngle;
                }
        ...
        if (estaDetrasDeAlgunEnemigo) 
        {
            ...
            else if (angle >= 160f && angle < 165f)
            {
                m_agent.AddReward(1.25f);
            }
            else if (angle >= 165f && angle < 170f)
            {
                m_agent.AddReward(1.5f);
            }
            else if (angle >= 170f && angle < 175f)
            {
                m_agent.AddReward(1.75f);
            }
            else if (angle >= 175f && angle <= 180f)
            {
                m_agent.AddReward(2f);
            }

            Debug.Log("ET Se escondio**");
